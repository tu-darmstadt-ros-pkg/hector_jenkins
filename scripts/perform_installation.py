...
with cd(HECTOR_TRACKER_INSTALL_PATH):
    try:
        result = subprocess.check_output('./update.sh', stderr=subprocess.STDOUT)
        ...
        result = subprocess.check_output(['wstool', 'update'], stderr=subprocess.STDOUT)
        print("Output of wstool update: \n" + result)
        result = subprocess.check_output(['catkin', 'build'], stderr=subprocess.STDOUT)
        print("Output of catkin build: \n" + result)
        if '[build] Failed: No packages failed.' not in result:
            sys.exit([1])
    except subprocess.CalledProcessError as error:
        print("Error while calling a command in perform_installation. Output: " + error.output)
        sys.exit([1])
